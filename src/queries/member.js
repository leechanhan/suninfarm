...
const MemberQuery = {
	// getLogin: ({ usr_id, usr_pw }, options = {}) => {
	// 	const { data, isLoading } = useCustomQuery({
	// 		queryKey: [...QueryKeys.진행중인_캠페인_목록],
	// 		queryFn: () => MemberQueryHandler.extractMemberLoginResponse({ usr_id, usr_pw }),
	// 		...options,
	// 	});
...
const MemberQueryHandler = {
	extractMemberLoginResponse: async ({ usr_id, usr_pw }) => {
		const { result = '' } = await MemberService.getLogin({ usr_id, usr_pw });
		return result;
	},
